...
class tx_dixeasylogin_oauth1 {
	function init($provider, $piVars) {
		...
		if (!class_exists($sigClass)) { $sigClass = '\DIX\OAuthSignatureMethod_HMAC_SHA1'; }
	...
	function getRequestToken() {
	  $req_req = \DIX\OAuthRequest::from_consumer_and_token($this->consumer, NULL, "GET", $this->provider['requestTokenUrl'], array());
	...
	function verifyLogin() { // get access token
	  ...
	  $acc_req = \DIX\OAuthRequest::from_consumer_and_token($this->consumer, $tokenObj, "GET", $this->provider['accessTokenUrl'], array());
	...
	function getUserInfo($accessTokenParams, &$error) {
		...
		$tokenObj = t3lib_div::makeInstance('OAuthConsumer', $this->oauth_token, $this->oauth_token_secret);
	  $req = \DIX\OAuthRequest::from_consumer_and_token($this->consumer, $tokenObj, "GET", $endpoint, array());
