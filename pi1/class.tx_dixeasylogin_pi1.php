...
require_once(t3lib_extMgm::extPath("dix_easylogin")."pi1/class.tx_dixeasylogin_div_hybrid_auth.php");
require_once(t3lib_extMgm::extPath("dix_easylogin")."pi1/class.tx_dixeasylogin_google_hybrid_auth.php");
require_once(t3lib_extMgm::extPath("dix_easylogin")."pi1/class.tx_dixeasylogin_facebook_hybrid_auth.php");
require_once(t3lib_extMgm::extPath("dix_easylogin")."pi1/class.tx_dixeasylogin_twitter_hybrid_auth.php");
require_once(t3lib_extMgm::extPath("dix_easylogin")."pi1/class.tx_dixeasylogin_xing_hybrid_auth.php");
...
class tx_dixeasylogin_pi1 extends tslib_pibase {
	...
	function main($content, $conf) {
		...
		if ($this->piVars['action'] == 'xrds') {
			...
			if ($loginType) {
				...
				switch ($provider['type']) {
					...
					case 'OPENID':
						...
						break;
					case 'GOOGLE_HA':
						$obj = t3lib_div::makeInstance('tx_dixeasylogin_google_hybrid_auth');
						break;
					case 'FACEBOOK_HA':
						$obj = t3lib_div::makeInstance('tx_dixeasylogin_facebook_hybrid_auth');
						break;
					case 'TWITTER_HA':
						$obj = t3lib_div::makeInstance('tx_dixeasylogin_twitter_hybrid_auth');
						break;
					case 'XING_HA':
						$obj = t3lib_div::makeInstance('tx_dixeasylogin_xing_hybrid_auth');
						break;
					default:
			...
			session_start(); //The session seems not started at this point, go ahead and start...
			if(isset($_SESSION['HA::CONFIG'])){ //Sniff the session for an existing Hybrid Auth...
				if(isset($_SESSION['HA::CONFIG']['config'])){
					...
					if(isset($config['providers'])){
						...
						$t3ProviderName = $provider[$providerName[0]]['t3_provider_name'];
						$loginType = $provider[$providerName[0]]['t3_provider_id'];
						switch ($t3ProviderName) {
							case 'GOOGLE_HA':
								$obj = t3lib_div::makeInstance('tx_dixeasylogin_google_hybrid_auth');
								break;
							case 'FACEBOOK_HA':
							...
							case 'XING_HA':
								...
								break;
							default:
								return("Undefined authentication method '{$t3ProviderName}'!");
						}
						
						$obj->init($this->providers[$loginType], $this->piVars);
						$error = $obj->main();
					}
				}
			}//Session property for Hybid Auth not set, go ahead...
