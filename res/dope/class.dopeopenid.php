	...
	*/
	public function filterUserInfo($arr_get)
	{
		foreach($arr_get as $key => $value){
			...
			if(stristr($key, 'openid_ext1_value') && isset($value[1])) { // e.g. google
				$this->arr_userinfo[$trimmed_key] = $value;
			}
			elseif(stristr($key, 'openid_ax_value') && isset($value[1])) { // e.g. yahoo
				$this->arr_userinfo[$trimmed_key] = $value;
			}
			elseif(stristr($key, 'sreg_') && array_key_exists($trimmed_key, $this->arr_ax_types)) {
				$this->arr_userinfo[$trimmed_key] = $value;
			}
		}
